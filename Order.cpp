#include <algorithm> // For std::find and std::any_of
...
void DeployOrder::validate() {
    ...
    if (std::find(playerTerritories.begin(), playerTerritories.end(), this->location) != playerTerritories.end()) {
        ...
        std::cout << "Invalid deploy order! This territory doesn't belong to you. Try again!" << std::endl;
    }
}
...
void DeployOrder::execute() {
    ...
    if (getValidity()) {
        ...
        std::cout << "Territory " << location->getName() << " now has " << location->getArmies() << " armies" <<std::endl;
    } else {
...
void AdvanceOrder::validate() {
    ...
    if (issuingPlayer == from->getOwner()) {
        std::cout << "Ownership of source territory confirmed: " << issuingPlayer->getName() << " owns " << from->getName() << "." << std::endl;
    } else {
    if (from->getOwner() != nullptr) { // check if issuing player owns the territorry
        std::cout << "Ownership mismatch: Issuing player " << issuingPlayer->getName()
                  << " does not won the territory. " << from->getOwner()->getName() << " owns the territory." << std::endl;
    } else {
        std::cout << "Ownership mismatch: Issuing player " << issuingPlayer->getName()
                  << " does not own the territory. The territory has no owner." << std::endl;
    }
    validity = false;
    return;
}
    
    // Check adjacency between source and target territories
    if (isAdjacent(from, to)) {
        std::cout << "Adjacency confirmed between " << from->getName() << " and " << to->getName() << "." << std::endl;
    } else {
        std::cout << "Adjacency check failed: " << from->getName() << " and " << to->getName() << " are not adjacent." << std::endl;
        validity = false;
        return;
    }
    
    // Check if armies to move are positive
    if (armies > 0) {
        std::cout << "Valid number of advancing armies - positive number: " << armies << std::endl;
    } else {
        std::cout << "Invalid number of armies: " << armies << ". Must be greater than zero." << std::endl;
        validity = false;
        return;
    }
    ...
    if (issuingPlayer == from->getOwner() && isAdjacent(from, to) && armies > 0) {
        ...
        if (issuingPlayer != targetPlayer) {
            if(!issuingPlayer->isDiplomacyPlayer(targetPlayer)){
            std::cout << "Advance order is valid! " << issuingPlayer->getName() << " is attacking " << targetPlayer->getName() << "." << std::endl;
            attacking = true;
            validity = true;  // Mark valid only after confirming it's not owned by the player
            } else {
                std::cout << issuingPlayer->getName() << " tried to attack player " << targetPlayer->getName() << ", with who he negociated peace. Invalid order" << std::endl;
            }
        } else {
            std::cout << "Issuing player is the target player, we will move " << from->getArmies() << " from source territory " << from->getName() << " to target territory " << to->getName() << std::endl;
            DeployOrder(issuingPlayer, from->getArmies(), to );
        }
    } else {
        std::cout <<from->getOwner()<< "Invalid advance order, make sure the path is clear!" << std::endl;
        validity = false;
    }
}
...
void AdvanceOrder::execute() {
    ...
    if (getValidity()) {
        std::cout << "The order is valid, time to advance! Let's move those armies!" << std::endl;

        if (attacking) {
            std::srand(static_cast<unsigned>(std::time(nullptr))); // Seed for randomness
            std::cout << "Attack will begin, engaging battle... wishing you luck !" << std::endl;
            ...
            while (to->getArmies() > 0 && remainingAttackers > 0) {
                ...
                if (randAttack >= 0.4) {
                    ...
                    std::cout << "Boom! An enemy unit of " << to->getName() << " falls!" << std::endl;
                }
                if (randDefend >= 0.3) {
                    remainingAttackers -= 1;
                    std::cout << "Ouch! "<< from->getName() << "'s attackers took a hit!" << std::endl;
                }
            }
            ...
            if (to->getArmies() == 0) {
                ...
                std::cout << to->getName() << " now has " << to->getArmies() << " armies on the territory." << std::endl;
                std::cout << from->getName() << " now has " << from->getArmies() << " armies on the territory." << std::endl;
            } else {
            ...
            std::cout << "Advancement executed. " << armies << " armies moved from " << from->getName() << " to " << to->getName() << "." << std::endl;
        }
    } else {
...
void BombOrder::validate() {
    ...
    if (location->getOwner() != issuingPlayer) {
        ...
        if (isAdjacent) {
            ...
            std::cout << "Invalid bomb order, too far to bomb! Find an adjacent territory." << std::endl;
            validity = false;
        }
    } else {
        std::cout << "Invalid bomb order, you can't bomb your own territory! Think twice before acting!" << std::endl;
        validity = false;
    }
}
...
void BombOrder::execute() {
	...
	if (getValidity()) {
		...
		std::cout << location->getName() << " now has: " << location->getArmies() << " armies left in the bombed territory" << std::endl;
...
void BlockadeOrder::validate() {
    ...
    if (location->getOwner() == issuingPlayer) {
        validity = true;
        std::cout << "Blockade order validated, issuing player owns the territory!" << std::endl;
    } else {
        validity = false;
        std::cout << "Invalid blockade order, target territory belongs to an enemy player!" << std::endl;
    }
}
...
void BlockadeOrder::execute() {
    ...
    if (getValidity()) {  // Ensuring that execute only proceeds if validated
        std::cout << "Blockade executed! The number of army units on the territory is doubled, Neutral Player now owns the territory." << std::endl;
        ...
        std::cout << "Blockade order execution failed! No effect..." << std::endl;
    }
}
...
void AirliftOrder::validate() {
    ...
    if (from->getOwner() == issuingPlayer && from->getOwner() == issuingPlayer) {
        if (armies > from->getArmies()) {
            validity = false;
            std::cout << "Invalid airlift order, cannot airlift more soldiers than available in the source territory!" << std::endl;
        } else {
            validity = true;
        ...
        validity = false;
        std::cout << "Invalid order. You can't airlift from an enemy territory, or no troops to move!" << std::endl;
    }
}
...
void NegotiateOrder::validate() {
    ...
    if (issuingPlayer != targetPlayer) {
        ...
        validity = false;
        std::cout << "Invalid negociation order, you cannot negotiate with yourself! Find a worthy opponent." << std::endl;
    }
}
